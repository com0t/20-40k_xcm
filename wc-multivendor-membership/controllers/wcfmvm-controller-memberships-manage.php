...
class WCFMvm_Memberships_Manage_Controller {
	...
	public function processing() {
	  ...
	  $wcfm_membership_manager_form_data = wc_clean( $wcfm_membership_manager_form_data );
	  	...
	  	// Creating new membership
			...
			if(!is_wp_error($new_membership_id)) {
				...
				if( isset( $_POST['free_thankyou_content'] ) ) {
					wcfm_update_post_meta( $new_membership_id, 'free_thankyou_content', wp_filter_post_kses( stripslashes( html_entity_decode( $_POST['free_thankyou_content'], ENT_QUOTES, 'UTF-8' ) ), 'post' ) );
				}
				
				// On Approval Thank You page content
				if( isset( $_POST['subscription_thankyou_content'] ) ) {
					wcfm_update_post_meta( $new_membership_id, 'subscription_thankyou_content', wp_filter_post_kses( stripslashes( html_entity_decode( $_POST['subscription_thankyou_content'], ENT_QUOTES, 'UTF-8' ) ), 'post' ) );
				}
				...
				if( isset( $_POST['subscription_welcome_email_content'] ) ) {
					wcfm_update_post_meta( $new_membership_id, 'subscription_welcome_email_content', wp_filter_post_kses( stripslashes( html_entity_decode( $_POST['subscription_welcome_email_content'], ENT_QUOTES, 'UTF-8' ) ), 'post' ) );
				}
