...
class WCFMvm_Frontend {
  ...
  function wcfmvm_vendor_manage_membrship_details( $vendor_id ) {
  	global $WCFM, $WCFMvm;
		...
		if( count( $wcfm_memberships_list ) >= 1 ) {
			...
			<button class="wcfm_modify_vendor_membership button" id="wcfm_modify_vendor_membership" data-memberid="<?php echo esc_attr($vendor_id); ?>"><?php _e( 'Update', 'wc-frontend-manager' ); ?></button>
  ...
  function wcfmvm_vendor_membership_user_setting_block( $user_id ) {
  	$membership_block_id = apply_filters( 'wcfm_membership_block_id', 'sm_profile_manage_form_membership_head' );
  	?>
  	<a href="<?php echo get_wcfm_profile_url(); ?>#<?php echo esc_attr($membership_block_id); ?>" class="page_collapsible page_collapsible_dummy" id="<?php echo esc_attr($membership_block_id); ?>"><label class="wcfmfa fa-user-plus"></label><?php _e( 'Membership', 'wc-multivendor-membership' ); ?><span></span></a>
  ...
  function wcfmvm_vendor_membership_user_profile() {
		...
		?>
		<div class="page_collapsible profile_manage_membership" id="<?php echo esc_attr($membership_block_id); ?>"><label class="wcfmfa fa-user-plus"></label><?php _e( 'Membership', 'wc-multivendor-membership' ); ?><span></span></div>
		<div class="wcfm-container">
  ...
  /**
    ...
    $uniqid = md5( uniqid() );
		...
		<form id="wcfm_membership_payment_form_stripe" class="wcfm wcfm_membership_payment_form wcfm_membership_payment_form_non_free" action="<?php echo esc_url($notify_url); ?>" method="post">
		  ...
		  <script src="https://checkout.stripe.com/checkout.js" class="stripe-button" data-key="<?php echo esc_attr($publishable_key); ?>"
																																									data-panel-label="<?php echo esc_attr($button_label); ?>"
																																									data-amount="<?php echo wcfmvm_membership_tax_price($price_in_cents); ?>"
																																									data-name="<?php echo esc_attr($title); ?>"
																																									data-description="<?php echo esc_attr($pay_description); ?>"
																																									data-label="<?php _e( 'Proceed', 'wc-multivendor-membership' ); ?>"
																																									data-currency="<?php echo esc_attr($payment_currency); ?>"
																																									data-locale="auto"
																																									></script>
			</div>
			...
			<input type="hidden" name="currency_code" value="<?php echo esc_attr($payment_currency); ?>">
			<input type="hidden" name="item_number" value="<?php echo esc_attr($membership_id); ?>">
			<input type="hidden" name="item_name" value="<?php echo esc_attr($title . ' - ' . $pay_description); ?>">
			<input type="hidden" value="<?php echo esc_attr($member_id); ?>" name="custom" />
		<?php	
		return true;
	}
}